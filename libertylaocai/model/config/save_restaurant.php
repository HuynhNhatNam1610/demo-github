...
try {
    ...
    // Xóa ảnh cũ không còn trong existing_images
    if (!empty($existing_images)) {
        $placeholders = implode(',', array_fill(0, count($existing_images), '?'));
        $sql = "DELETE FROM anhnhahang WHERE image NOT IN ($placeholders)";
        $stmt = mysqli_prepare($conn, $sql);
        $types = str_repeat('s', count($existing_images));
        mysqli_stmt_bind_param($stmt, $types, ...$existing_images);
        if (!mysqli_stmt_execute($stmt)) {
            error_log("SQL error deleting anhnhahang: " . mysqli_error($conn));
            throw new Exception('Lỗi khi xóa ảnh cũ');
        }
        mysqli_stmt_close($stmt);
        error_log("Deleted anhnhahang not in existing_images");
    } else {
        $sql = "DELETE FROM anhnhahang WHERE id_topic = 12";
        $stmt = mysqli_prepare($conn, $sql);
        if (!mysqli_stmt_execute($stmt)) {
            error_log("SQL error deleting all anhnhahang: " . mysqli_error($conn));
            throw new Exception('Lỗi khi xóa ảnh cũ');
        }
        mysqli_stmt_close($stmt);
        error_log("Deleted all anhnhahang for id_topic: 12");
    }

    // Xử lý ảnh
    $image_paths = [];
    if (isset($_FILES['image']) && count($_FILES['image']['name']) > 0) {
        $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/libertylaocai/view/img/uploads/nhahang/';
        $relative_path = 'uploads/nhahang/';
    ...
    foreach ($image_paths as $image) {
        $sql = "INSERT INTO anhnhahang (image, active, created_at, id_topic) VALUES (?, 1, NOW(), 12)";
        $stmt = mysqli_prepare($conn, $sql);
    ...
    $response['message'] = 'Restaurant info saved successfully';
} catch (Exception $e) {
    mysqli_rollback($conn);
    $response['message'] = 'Error: ' . $e->getMessage();
    error_log("Error in save_restaurant.php: " . $e->getMessage());
}
